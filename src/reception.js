...
async function renderAll() {
    ...
    if (key !== 'blocked') {
      ...
      sel.innerHTML = maids
          .map((m) => {
            ...
            return `<option value="${m.id || m.email}" ${selected} ${disabled}>${labelText}</option>`;
          })
          .join("");
  ...
  if (getStatusKey(r.status) !== 'blocked') {
    ...
    if (getStatusKey(r.status) === "blocked") {
      ...
      btn.addEventListener("click", async () => {
        const ok = await confirmAction(`¿Habilitar la habitación ${r.id}? Se marcará como "Sucio".`);
        if (!ok) return;
  ...
  nextM.addEventListener('click', () => { if (maidsStart + MAIDS_PER_PAGE < totalMaids) { maidsPage++; renderAll(); } });
  const infoM = document.createElement('span');
  infoM.textContent = `Página ${maidsPage + 1} / ${Math.max(1, Math.ceil(totalMaids / MAIDS_PER_PAGE))}`;
  ...
  if (reportsPagerEl) {
    ...
    prevReports.addEventListener('click', () => { reportsPage = Math.max(0, reportsPage - 1); renderAll(); });
    ...
    nextReports.addEventListener('click', () => { if (reportsStart + REPORTS_PER_PAGE < totalReports) { reportsPage++; renderAll(); } });
    const infoReports = document.createElement('span');
    infoReports.textContent = `Página ${reportsPage + 1} / ${Math.max(1, Math.ceil(totalReports / REPORTS_PER_PAGE))}`;
...
async function addRoomModal() {
  ...
  const maidOptions = allMaids
    .map(m => {
      ...
      const label = (m.name || id) + ((m.status || '').toLowerCase().includes('no') ? ' (No disponible)' : '');
      const disabled = ((m.status || '').toLowerCase().includes('no')) ? 'disabled' : '';
  ...
  document.getElementById("saveRoom").onclick = async () => {
    ...
    await put("rooms", { id, status: status || "Limpia", maid: maidId, rented: !!rented });
